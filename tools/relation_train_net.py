...
def train(cfg, local_rank, distributed, logger):
    ...
    best_model_info = {'iteration':-1, 'checkpoint':None, 'result':0}
    ...
    for iteration, this_batch in enumerate(train_data_loader, start_iter):
        if cfg.WSVL.OFFLINE_OD:  # offline object detector detection results
            if cfg.WSVL.USE_UNITER:
                if cfg.WSVL.USE_GROUND:
                    det_feats, det_dists, det_boxes, targets, _, images, gd_triplets, gd_subjects, gd_objects, gd_obj_labels, det_tag_ids, det_norm_pos = this_batch
                else:
                    det_feats, det_dists, det_boxes, targets, _, images,det_tag_ids, det_norm_pos = this_batch
                    gd_triplets, gd_subjects, gd_objects, gd_obj_labels = None, None, None, None
            else:
                if cfg.WSVL.USE_GROUND:
                    det_feats, det_dists, det_boxes, targets, _, images, gd_triplets, gd_subjects, gd_objects, gd_obj_labels = this_batch
                else:
        ...
        if cfg.WSVL.OFFLINE_OD:  # offline object detector detection results
            if cfg.WSVL.USE_UNITER:
                ...
                if cfg.WSVL.USE_GROUND:
                    gd_triplets = [triplet.to(device) for triplet in gd_triplets]
                    gd_subjects = [sub.to(device) for sub in gd_subjects]
                    gd_objects = [obj.to(device) for obj in gd_objects]
                    gd_obj_labels = [lbl.to(device) for lbl in gd_obj_labels]
                ...
                loss_dict = model(images, targets, det_feats=det_feats, det_dists=det_dists, det_boxes=det_boxes,\
                    gd_triplets=gd_triplets, gd_subjects=gd_subjects, gd_objects=gd_objects, gd_obj_labels=gd_obj_labels,\
        ...
        if cfg.SOLVER.TO_VAL and iteration % cfg.SOLVER.VAL_PERIOD == 0:
            ...
            if val_result > best_model_info['result']:
                best_model_info['iteration'] = iteration
                best_model_info['result'] = val_result

               
                best_data = {}
                best_data['model'] = copy.deepcopy(model.state_dict())
                best_data['optimizer'] = copy.deepcopy(optimizer.state_dict())
                best_data['scheduler'] = copy.deepcopy(scheduler.state_dict())
                best_model_info['checkpoint'] = best_data

                torch.save(best_model_info['checkpoint'], cfg.OUTPUT_DIR+"/model_best.pth")
    ...
    logger.info("Save the best model: iteration: {iteration}, reall@100: {result}".format(**best_model_info))
