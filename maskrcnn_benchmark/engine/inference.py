    ...
    for _, batch in enumerate(tqdm(data_loader)):
        with torch.no_grad():
            if offline_od: # offline object detector
                if use_uniter:
                    det_feats, det_dists, det_boxes, targets, image_ids, images, gd_triplets, gd_subjects, gd_objects, gd_obj_labels, det_tag_ids, det_norm_pos = batch
                    ...
                    det_feats, det_dists, det_boxes, targets, image_ids, images, gd_triplets, gd_subjects, gd_objects, gd_obj_labels = batch
                    targets = [target.to(device) for target in targets]
                    det_feats = [det_feat.to(device) for det_feat in det_feats]
                    det_dists = [det_dist.to(device) for det_dist in det_dists]
                    det_boxes = [det_box.to(device) for det_box in det_boxes]   
