...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                ...
                use_cap_trip=False, caption_label_file=None, ground_file=None):
        """
        ...
        ground_file_path = ground_file

        if ground_file_path is not None:
            if not self.use_cap_trip:
                self.grounding_triplets, self.grounding_subjects, self.grounding_objects, self.ground_keys, self.det_objects_matched_labels = self.get_grounding_results(ground_file_path)
            else:
                self.grounding_triplets, self.grounding_subjects, self.grounding_objects, self.ground_keys, self.det_objects_matched_labels = self.get_grounding_cap2sg_results(ground_file_path)
        else:
            self.grounding_triplets = None
    ...
    def __getitem__(self, index):
        if not self.offline_OD: # use online object detector 
            ...
            if not self.use_uniter: # use default predictor
                ...
                if self.grounding_triplets is None:
                    return det_feat, det_dist, det_box, target, index, img
                else:
                    # load file
                    if int(img_id) in self.grounding_triplets.keys():
                        gd_triplets = self.grounding_triplets[int(img_id)]
                        gd_subjects = self.grounding_subjects[int(img_id)]
                        gd_objects = self.grounding_objects[int(img_id)]
                        gd_obj_labels = self.det_objects_matched_labels[int(img_id)]
                    else:
                        gd_triplets = torch.zeros((1,3), dtype=torch.int)
                        gd_subjects = torch.zeros((1,36), dtype=torch.int)
                        gd_objects = torch.zeros((1,36), dtype=torch.int)
                        gd_obj_labels = torch.zeros((36,), dtype=torch.int)

                    return det_feat, det_dist, det_box, target, index, img, gd_triplets, gd_subjects, gd_objects,  gd_obj_labels
                ...
                if self.grounding_triplets is None:
                    ...
                    return det_feat, det_dist, det_box, target, index, img, gd_triplets, gd_subjects, gd_objects,  gd_obj_labels, det_tag_ids, det_norm_pos
    ...
    def get_grounding_cap2sg_results(self, g_path):

        def filter_top_boxes(boxes, scores, k=3):
            scores = np.array(scores)
            ids = np.argsort(scores).tolist()[::-1][:k]
            output_boxes = [boxes[i] for i in ids]
            output_scores = [scores[i] for i in ids]
        ...
        heatmap_subjects = {}
        heatmap_objects = {}
        det_objects_matched_labels = {}

        with open(g_path, 'rb')as fr:
            result_metas = pkl.load(fr)

        for image_id in result_metas.keys():
            meta = result_metas[image_id]
            gt_rels = meta['relations']
            gt_classes = meta['gt_classes']
            gd_boxes = meta['gd_boxes']
            gd_scores = meta['gd_scores']

            if len(gt_rels) == 0:
                continue

            grounding_triplets[image_id] = []
            heatmap_subjects[image_id] = []
            heatmap_objects[image_id] = []
            det_objects_matched_labels[image_id] = np.zeros((36, 151), dtype=int)
            matched_object_labels = [set() for i in range(36)]

            # gd_scores = softmax(np.array(gd_scores), -1)

            # caption_boxes = meta['caption_boxes']
            for _, (triplet) in enumerate(gt_rels):
                sub, obj, rel_id = triplet
                sub_id = gt_classes[sub]
                obj_id = gt_classes[obj]
                sub_mask = np.zeros((36,), dtype=int)
                f_sub_box, f_sub_score = filter_top_boxes(gd_boxes[sub], gd_scores[sub])
                # f_sub_box = gd_boxes[sub]
                for ff in f_sub_box:
                    matched_object_labels[ff].add(sub_id)
                sub_mask[f_sub_box] = 1
                # sub_mask[f_sub_box] = gd_scores[sub]
                heatmap_subjects[image_id].append(sub_mask)
                obj_mask = np.zeros((36,), dtype=int)
                f_obj_box, f_obj_score = filter_top_boxes(gd_boxes[obj], gd_scores[obj])
                # f_obj_box = gd_boxes[obj]
                for ff in f_obj_box:
                    matched_object_labels[ff].add(obj_id)
                obj_mask[f_obj_box] = 1
                # obj_mask[f_obj_box] = gd_scores[obj]
                heatmap_objects[image_id].append(obj_mask)
                # sub_id = box_index[0]
                # obj_id = box_index[1]

                grounding_triplets[image_id].append([sub_id, obj_id, rel_id])
            
            for ii in range(len(matched_object_labels)):

                matched = list(matched_object_labels[ii])
                det_objects_matched_labels[image_id][ii][matched] = 1
                
                # grounding_boxes[image_id].append(np.concatenate((sub_box, obj_box), -1))
            grounding_triplets[image_id] = torch.from_numpy(np.array(grounding_triplets[image_id]))
            heatmap_subjects[image_id]= torch.from_numpy(np.array(heatmap_subjects[image_id]))
            heatmap_objects[image_id] = torch.from_numpy(np.array(heatmap_objects[image_id]))
            det_objects_matched_labels[image_id] = torch.from_numpy(det_objects_matched_labels[image_id])
        ground_keys = grounding_triplets.keys()
        # pdb.set_trace()

        return grounding_triplets, heatmap_subjects, heatmap_objects, ground_keys, det_objects_matched_labels
    
    # def get_grounding_process_results(self, g_path):
    #     with open(g_path, 'rb')as fr:
    #         result_metas = pkl.load(fr)
    #     grounding_triplets = result_metas['gd_triplets']
    #     grounding_subjects = result_metas['gd_subjects']
    #     grounding_objects = result_metas['gd_objects']
    #     det_objects_matched_labels = result_metas['objects_matched_labels']
    #     ground_keys = grounding_triplets.keys()
    #     # print('dfsdgdsfgdsfgfdhsdfh')
    #     # print(list(ground_keys)[:100])

    #     return grounding_triplets, grounding_subjects, grounding_objects, ground_keys, det_objects_matched_labels
        

    def get_grounding_results(self, g_path):
        ...
        with open(g_path, 'rb')as fr:
            ...
            for meta in result_metas:
                ...
                gt_rels = meta['gt_rels']
                gt_classes = meta['gt_classes']
                # det_boxes = det_box_file[str(image_id)] 
                grounding_triplets[image_id] = []
                # grounding_boxes[image_id] = []
                heatmap_subjects[image_id] = []
                heatmap_objects[image_id] = []
                det_objects_matched_labels[image_id] = np.zeros((36, 151), dtype=int)
                matched_object_labels = [set() for i in range(36)]
                ...
                gt_classes = meta['gt_classes']
                # caption_boxes = meta['caption_boxes']
                for _, (triplet, sub_boxes, obj_boxes, sub_scores, obj_scores) in enumerate(zip(gt_rels,meta['subject_boxes'], meta['object_boxes'], meta['subject_scores'], meta['object_scores'])):
                    sub, obj, rel_id = triplet
                    sub_id = gt_classes[sub]
                    obj_id = gt_classes[obj]
                    sub_mask = np.zeros((36,), dtype=int)
                    f_sub_box, f_sub_score = filter_top_boxes(sub_boxes, sub_scores)
                    for ff in f_sub_box:
                        matched_object_labels[ff].add(sub_id)
                    sub_mask[f_sub_box] = 1
                    heatmap_subjects[image_id].append(sub_mask)
                    obj_mask = np.zeros((36,), dtype=int)
                    f_obj_box, f_obj_score = filter_top_boxes(obj_boxes, obj_scores)
                    for ff in f_obj_box:
                        matched_object_labels[ff].add(obj_id)
                    obj_mask[f_obj_box] = 1
                    heatmap_objects[image_id].append(obj_mask)
