    ...
    def __call__(self, batch):
        ...
        if len(transposed_batch) == 3: # use online object detector
            ...
            if len(transposed_batch) == 6:
                ...
                gd_triplets = transposed_batch[6]
                gd_subjects = transposed_batch[7]
                gd_objects = transposed_batch[8]
                
                gd_obj_labels = transposed_batch[9]

                if len(transposed_batch) >10:
                    det_tag_ids = transposed_batch[10] # tuple of List, the elements have variable length
                    det_norm_pos = transposed_batch[11]
                    return det_feats, det_dists, det_boxes, targets, img_ids, images, gd_triplets, gd_subjects, gd_objects, gd_obj_labels, det_tag_ids, det_norm_pos  
                else:
                    return det_feats, det_dists, det_boxes, targets, img_ids, images, gd_triplets, gd_subjects, gd_objects,gd_obj_labels           
