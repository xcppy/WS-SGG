...
class ROIBoxHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, gd_obj_labels=None):
        """
        Arguments:
                    ...
                    if self.training or not self.cfg.TEST.CUSTUM_EVAL:
                        ...
                        if self.cfg.WSVL.HARD_DICT_MATCH is not None: # use class dictionary to match gt objects
                            if gd_obj_labels is None:
                                ...
                                proposals = self.samp_processor.assign_label_to_proposals_by_dict_and_gd(proposals, targets, self.cls_dict, gd_obj_labels)
