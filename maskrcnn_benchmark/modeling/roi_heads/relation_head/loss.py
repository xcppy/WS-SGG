...
def SoftCrossEntropy(inputs, targets, reduction='mean'):
    log_likelihood = -F.log_softmax(inputs, dim=-1)
    batch = inputs.shape[0]
    if reduction == 'mean':
        loss = torch.sum(torch.mul(log_likelihood, targets)) / batch
    if reduction == 'sum':
        loss = torch.sum(torch.mul(log_likelihood, targets))
    return loss 
