...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None, det_feats=None, det_dists=None,\
        gd_triplets=None, gd_subjects=None, gd_objects=None, det_tag_ids=None, det_norm_pos=None):
        """
            ...
            with torch.no_grad():
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    ...
                    if gd_triplets is None:
                        ...
                        proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.detect_ground_data_relsample(proposals, targets, gd_triplets, gd_subjects, gd_objects, \
                                use_bert=self.cfg.WSVL.USE_UNITER, dict_match=self.dict_match)
        else:
        ...
        if self.cfg.WSVL.USE_UNITER: # use uniter as relation predictor 
            ...
            # loss_relation, loss_ensemble, loss_refine = self.loss_evaluator(proposals, rel_labels, relation_logits, refine_logits, soft_labels)
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
            ...
            # output_losses = dict(loss_rel=loss_relation, loss_refine_obj=loss_refine, loss_ensemble=loss_ensemble)
