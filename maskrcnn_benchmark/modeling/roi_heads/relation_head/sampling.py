...
class RelationSampling(object):
    ...
    def detect_ground_data_relsample(self, proposals, targets, gd_triplets, gd_subjects, gd_objects, use_bert=False, dict_match=False):
        self.num_pos_per_img = int(self.batch_size_per_image * self.positive_fraction)
        rel_idx_pairs = []
        rel_labels = []
        rel_sym_binarys = []
        rel_scores = []
        for img_id, (proposal, target, g_triplets, prp_subjects, prp_objects) in enumerate(zip(proposals, targets, gd_triplets, gd_subjects, gd_objects)):
            device = proposal.bbox.device
            ...
            inter_triplets, rel_possibility, overlap_possibility = self.generate_ground_data_results(device, g_triplets, prp_lab, prp_subjects, prp_objects, rel_possibility, overlap_possibility)
            if use_bert:
            ...
            img_rel_triplets, binary_rel = self.rel_gd_sampling(device, inter_triplets, rel_possibility, overlap_possibility)
            if use_bert:
                ...
                # !!! TO DO REL SCORES FOR THESE BG RELATIONS.
    ...
    def rel_gd_sampling(self, device, g_triplets, rel_possibility, ious=None):
        """
        ...
        num_prp = rel_possibility.shape[0]
        binary_rel = torch.zeros((num_prp, num_prp), device=device).long()

        fg_rel_triplets = g_triplets
        # fg_rel_labels = g_triplets[:,2]
        ...
        if len(fg_rel_triplets) == 0:
            ...
            prp_head_idxs = fg_rel_triplets[:,0]
            prp_tail_idxs = fg_rel_triplets[:,1]
            binary_rel[prp_head_idxs, prp_tail_idxs] = 1
            binary_rel[prp_tail_idxs, prp_head_idxs] = 1
        ...
        # bg_rel_scores = torch.ones(bg_rel_inds.shape[0], dtype=torch.float, device=device)
        ...
        if bg_rel_triplets.shape[0] > 0:
            ...
            # bg_rel_scores = bg_rel_scores[perm]
        else:
            bg_rel_triplets = torch.zeros((0, 3), dtype=torch.int64, device=device)
            # bg_rel_scores = torch.ones((1), dtype=torch.float, device=device)
    ...
    def generate_ground_data_results(self, device, parsed_triplets, prp_labels, heat_subjects, heat_objects,  rel_possibility, overlap_possibility=None):
        # pdb.set_trace()
        heat_triplets = []
        # tgt_head_idxs = parsed_triplets[:, 0].contiguous().view(-1)
        # tgt_tail_idxs = parsed_triplets[:, 2].contiguous().view(-1)
        for _, (triplet, heat_head_mask, heat_tail_mask) in enumerate(zip(parsed_triplets, heat_subjects, heat_objects)) :
            # print('ahahaha')
            sub, obj, pred = triplet # find the box
            # print('prp_labels == sub', (prp_labels == sub).shape)
            # print('(heat_head_mask > 0)', (heat_head_mask > 0).shape)
            if len(heat_head_mask) != len(prp_labels):
                heat_head_mask = heat_head_mask[:len(prp_labels)]
                heat_tail_mask = heat_tail_mask[:len(prp_labels)]
            prp_head_idxs = torch.nonzero((prp_labels == sub) & (heat_head_mask > 0)).view(-1)
            prp_tail_idxs = torch.nonzero((prp_labels == obj) & (heat_tail_mask > 0)).view(-1)
            # prp_head_idxs = torch.nonzero(heat_head_mask > 0).view(-1)
            # prp_tail_idxs = torch.nonzero(heat_tail_mask > 0).view(-1)
            # ## 
            # heat_head_idxs = torch.nonzero(heat_head_mask > 0).view(-1)
            # heat_tail_idxs = torch.nonzero(heat_tail_mask > 0).view(-1)
            ...
            fg_labels = torch.tensor([pred]*prp_tail_idxs.shape[0], dtype=torch.int64, device=device).view(-1,1)
            ...
            if fg_rel_i.shape[0] > 0:
                heat_triplets.append(fg_rel_i)

            if overlap_possibility is not None:
                overlap_possibility[fg_rel_i[:,0], fg_rel_i[:,1]] = 0

        # if len(heat_triplets) == 0:
        #     heat_triplets = torch.zeros((0, 3), dtype=torch.int64, device=device)
        # else:
        #     heat_triplets = cat(heat_triplets, dim=0).to(torch.int64)
        #     if heat_triplets.shape[0] > self.num_pos_per_img:
        #         perm = torch.randperm(heat_triplets.shape[0], device=device)[:self.num_pos_per_img]
        #         heat_triplets = heat_triplets[perm] 

        return heat_triplets, rel_possibility, overlap_possibility
