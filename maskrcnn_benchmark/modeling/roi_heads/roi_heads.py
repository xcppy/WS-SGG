...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None, logger=None, det_feats=None, det_dists=None, det_boxes=None,\
        gd_triplets=None, gd_subjects=None, gd_objects=None, gd_obj_labels=None,det_tag_ids=None, det_norm_pos=None):
        losses = {}
        if not self.cfg.WSVL.OFFLINE_OD: # online detector
            ...
            _, detections, _ = self.box(features, det_boxes, targets, gd_obj_labels) # assign labels to offline bbox
        ...
        if self.cfg.MODEL.RELATION_ON:
            ...
            x, detections, loss_relation = self.relation(features, detections, targets, logger, det_feats=det_feats, det_dists=det_dists,\
                                                            gd_triplets=gd_triplets, gd_subjects=gd_subjects, gd_objects=gd_objects,\
